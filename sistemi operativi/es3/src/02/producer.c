...
void initMemory() {
    ...
    if(fd_shm==-1)
        handle_error("initMemory_shm_open_producer");
        
    if(ftruncate(fd_shm,sizeof(struct shared_memory))!=0)
        handle_error("initMemory_ftruncate_producer");
    myshm_ptr=mmap(0,sizeof(struct shared_memory),PROT_READ|PROT_WRITE, MAP_SHARED, fd_shm,0);
    if(myshm_ptr==MAP_FAILED)
        handle_error("initMemory_map_producer");
    if(memset(myshm_ptr, 0, sizeof(struct shared_memory))==NULL)
        handle_error("memset");
    //=================================
...
void closeMemory() {
    ...
    if(munmap(myshm_ptr,sizeof(struct shared_memory))!=0)
        handle_error("[@producer] munmap");
    
    if(close(fd_shm)==-1)
        handle_error("[@producer] fd_shm");
    if(shm_unlink(SH_MEM_NAME)==-1)
        handle_error("[@producer] shm_unlink");
    ///=====================================
}
