...
void* performTransactions(void* x) {
    ...
    while (args->numOps > 0) {
        ...
        //=============
        if(sem_wait(&celle_disponibili)){
            handle_error("sem_wait producer");
        }
         
        if(sem_wait(&s1)){
            handle_error("sem_wait_s_producer");
        }
        ...
        //==============
        if(sem_post(&s1)){
            handle_error("sem_post_s_producer");
        }
        if(sem_post(&consumabili)){
            handle_error("sem_post_producer");
        }
        //==============
...
void* processTransactions(void* x) {
    ...
    while (args->numOps > 0) {
        ...
        //=======
        if(sem_wait(&consumabili)){
            handle_error("sem_wait_consumer");
        }
        if(sem_wait(&s2)){
            handle_error("sem_wait_s_consumer");
        }
        //=======
        ...
        //=======
        if(sem_post(&s2)){
            handle_error("sem_post_s_consuner");
        }
        
        if(sem_post(&celle_disponibili)){
            handle_error("sem_post_consumer");
        }
...
int main(int argc, char* argv[]) {
    ...
    //================
    //sem_init(&s,0,1);
    sem_init(&celle_disponibili, 0, BUFFER_SIZE);
    sem_init(&consumabili,0, 0);
    sem_init(&s1,0,1);
    sem_init(&s2,0,1);
