...
void closeAndDestroySemaphores() {
    ...
    if(sem_close(sem_cs)==-1){
        handle_error("sem_close_cs_consumer");
    }
    if(sem_close(sem_filled)==-1){
        handle_error("sem_close_filled_consumer");
    }
    if(sem_close(sem_empty)==-1){
        handle_error("sem_close_empty_consumer");
    }
    // TODO: then unlink them, handling errors using the handle_error() macro
    if(sem_unlink(SEMNAME_CS)==-1){
        handle_error("sem_unlink_cs_consumer");
    }
    if(sem_unlink(SEMNAME_EMPTY)==-1){
        handle_error("sem_unlink_empty_consumer");
    }
    if(sem_unlink(SEMNAME_FILLED)==-1){
        handle_error("sem_unlink_filled_consumer");
    }
    ...
    while (numOps > 0) {
        ...
        * semaphore, and the second by waiting on the sem_cs semaphore. */
        ...
        if(sem_wait(sem_filled)==-1){
            handle_error("sem_wait_filled_consumer");
        }
        if(sem_wait(sem_cs)==-1){
            handle_error("sem_wait_cs_consumer");
        }
        //=====
        ...
        if(sem_post(sem_cs)==-1){
            handle_error("sem_post_cs_consumer");
        }
        if(sem_post(sem_empty)==-1){
            handle_error("sem_post_empty_consumer");
        }
