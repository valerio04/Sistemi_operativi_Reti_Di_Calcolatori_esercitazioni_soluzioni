...
void* client2(void* arg_ptr) {
    ...
    sem_wait(&semaphor_main);
    numero_thread_attuali-=1;
    sem_post(&semaphor_main);
...
int main(int argc, char* argv[]) {
    ...
    //======= varinate 2
    numero_thread_attuali=0;  
    
    sem_init(&semaphor_main, 0, 1);
    //=====================
    ...
    while(1) {
        ...
        //if (input_char == EOF) break; valido per cariante 0 e variante 1
        if(input_char == EOF){
            printf("in fase di chiusra, aspetto che finiscono tutti...");
            while(numero_thread_attuali!=0){
                sleep(1);//cosi che non occupo inutilente cpu, faccio un controll al secondfdo
            }
            break;
        } 
        ...
        for (i = 0; i < THREAD_BURST; ++i) {
            ...
            int ret=pthread_create(&thread_handle, NULL, client2, args);
            
            if (ret) {  //cambiare nome funz a seconda variante
