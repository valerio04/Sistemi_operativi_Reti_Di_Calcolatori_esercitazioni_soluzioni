...
int write_to_pipe(int fd, const void *data, size_t data_len) {
    ...
    while(num_scritti<data_len){

        int ret=write(fd,data+num_scritti,data_len-num_scritti);
        if (ret==-1){
            if(errno==EINTR) continue;
            if (errno == EPIPE) handle_error("write: lettori chiusi (EPIPE)");
            handle_error("write, errore");
        }
...
int read_from_pipe(int fd, void *data, size_t data_len) {
    ...
    int num_letti=0;
    while(num_letti<data_len){

        int ret=read(fd,data+num_letti,data_len-num_letti);
        if(ret==-1){
            if(errno==EINTR) continue;
            handle_error("read");
        }
        ...
        if(ret==0)
            handle_error("read, writer chiusi");
